...
class Bot:
    ...
    def create_timer(self, timeout_secs, callback, *args, **kwargs):
        ...
        if args:
            raise TypeError(
                f"create_timer() takes 2 positional arguments but {len(args) + 2} were given"
            )
    ...
    def create_countdown(self, timeout_secs, callback, *args, **kwargs):
        ...
        if args:
            raise TypeError(
                f"create_countdown() takes 2 positional arguments but {len(args) + 2} were given"
            )
        ...
        def wrapper(context):
            ...
            try:
                ...
                traceback.print_exception(
                    type(error), error, error.__traceback__, file=sys.stderr, limit=-3
                )
    ...
    def reply_on_message(self, callback, *args, **kwargs):
        ...
        if args:
            raise TypeError(
                f"reply_on_message() takes 1 positional argument but {len(args) + 1} were given"
            )
